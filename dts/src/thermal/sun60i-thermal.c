...
static int sun60i_thermal_get_temp(struct thermal_zone_device *tz, int *temp)
{
	struct sun60i_thermal_sensor *sensor = thermal_zone_device_priv(tz);
	struct sun60i_thermal *ths = sensor->ths;
	int sensor_id = sensor->id;
	u32 raw_temp;

	raw_temp = sun60i_thermal_read_temp(ths, sensor_id);

	if (ths->calibrated)
		*temp = (raw_temp - ths->cali_data[sensor_id]) * 412;
	else
...
static int sun60i_thermal_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < SUN60I_THERMAL_NUM_SENSORS; i++) {
		struct sun60i_thermal_sensor *sensor;
		...
		ths->tzd[i] = devm_thermal_of_zone_register(dev, i,
					sensor, &sun60i_thermal_ops);
		if (IS_ERR(ths->tzd[i])) {
